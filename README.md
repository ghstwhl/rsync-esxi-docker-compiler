...
## usage
In order to generate the statically compiled rsync for ESXi 6.5/6.7/7.0, use one of the below methods.  Both methods will place a copy of the statically linked `rsync` binary on your local machine.  You can then copy this `rsync` binary to the remote ESXi system, most likely into the /bin/ directory.  Be sure to chmod +x /bin/rsync on the remote ESXi system.
